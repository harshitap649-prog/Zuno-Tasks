...
/**
 * Hideout.tv Offerwall Component
 * Instant approval for video watching - earn points by watching videos
 */
export default function HideoutTvOfferwall({ apiKey, userId, offerwallUrl, onClose, onComplete }) {
  ...
  const buildOfferwallUrl = useCallback(() => {
    ...
    // Hideout.tv format: https://www.hideout.tv/offers?pub=API_KEY&sub=USER_ID
    // Alternative: https://www.loot.tv/offers?pub=API_KEY&sub=USER_ID (same company)
    if (apiKey) {
      return `https://www.hideout.tv/offers?pub=${apiKey}&sub=${userId}`;
    }
    return null;
  ...
  const awardPointsForOffer = useCallback(async (offerId, rewardPoints) => {
    ...
    if (rewardPoints > 0 && userId) {
      try {
        console.log(`Awarding ${rewardPoints} points for Hideout.tv video completion: ${offerId}`);
  ...
  useEffect(() => {
    const handleMessage = async (event) => {
      if (!event.origin.includes('hideout.tv') && !event.origin.includes('loot.tv')) {
        return;
      }
      console.log('Received message from Hideout.tv:', event.origin, event.data);

      if (event.data && typeof event.data === 'object') {
        const offerId = event.data.offer_id || event.data.id || event.data.transaction_id || `hideout_${Date.now()}`;
        const rewardPoints = parseInt(event.data.reward || event.data.amount || event.data.payout || event.data.points || 0);
        const isComplete = event.data.type === 'offer_complete' || event.data.event === 'offer_complete' || event.data.status === 'complete' || event.data.completed === true || event.data.video_completed === true;
  ...
  if (!finalOfferwallUrl) {
    return (
      <div className="p-8 text-center">
        <p className="text-gray-600 mb-4">Hideout.tv is not configured. Please add your API key or offerwall URL in Admin Panel.</p>
      </div>
    );
  }
  ...
  const handleOpenInNewTab = () => {
    try {
      ...
      if (newWindow && !newWindow.closed) {
        ...
        console.log('Hideout.tv offerwall opened in new tab, tracking for completion...');
      }
    } catch (error) {
      console.error('Error opening Hideout.tv offerwall:', error);
  ...
  return (
    <div className="w-full h-full">
      <div className="mb-4 p-4 bg-gradient-to-r from-pink-50 to-rose-50 border-2 border-pink-200 rounded-lg">
        <div className="flex items-start gap-3">
          <div className="flex-shrink-0 text-2xl">💡</div>
          <div className="flex-1">
            <p className="text-sm text-pink-800 mb-2 font-semibold">
              Watch videos and earn points automatically!
            </p>
            <p className="text-xs text-pink-700 mb-2">
              ✓ Click "Open in New Tab" for the best experience.
              <br />✓ Videos play automatically and points are awarded after completion.
      ...
      <div className="mb-4 flex justify-center">
        <button
          ...
          className="bg-pink-600 hover:bg-pink-700 text-white py-2 px-6 rounded-lg flex items-center gap-2 font-semibold"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
          </svg>
          Open Videos in New Tab (Recommended)
        </button>
      </div>
      ...
      <iframe
        ...
        title="Hideout.tv Video Offerwall"
