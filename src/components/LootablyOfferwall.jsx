...
/**
 * Lootably Offerwall Component
 * Instant approval for surveys, apps, videos, and more
 */
export default function LootablyOfferwall({ apiKey, userId, offerwallUrl, onClose, onComplete }) {
  ...
  const buildOfferwallUrl = useCallback(() => {
    ...
    // Lootably format: https://www.lootably.com/offers?pub=API_KEY&sub=USER_ID
    if (apiKey) {
      return `https://www.lootably.com/offers?pub=${apiKey}&sub=${userId}`;
    }
    return null;
  ...
  const awardPointsForOffer = useCallback(async (offerId, rewardPoints) => {
    ...
    if (rewardPoints > 0 && userId) {
      try {
        console.log(`Awarding ${rewardPoints} points for Lootably offer completion: ${offerId}`);
  ...
  useEffect(() => {
    const handleMessage = async (event) => {
      if (!event.origin.includes('lootably.com')) {
        return;
      }
      console.log('Received message from Lootably:', event.origin, event.data);

      if (event.data && typeof event.data === 'object') {
        const offerId = event.data.offer_id || event.data.id || event.data.transaction_id || `lootably_${Date.now()}`;
  ...
  if (!finalOfferwallUrl) {
    return (
      <div className="p-8 text-center">
        <p className="text-gray-600 mb-4">Lootably is not configured. Please add your API key or offerwall URL in Admin Panel.</p>
      </div>
    );
  }
  ...
  const handleOpenInNewTab = () => {
    try {
      ...
      if (newWindow && !newWindow.closed) {
        ...
        console.log('Lootably offerwall opened in new tab, tracking for completion...');
      }
    } catch (error) {
      console.error('Error opening Lootably offerwall:', error);
